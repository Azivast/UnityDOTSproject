...
namespace ECSBoids
{
    public partial struct BoidSystem : ISystem
    {
        ...
        {
            var boidPositions = new List<float3>();
            var boidVelocities = new List<float3>();
            foreach (RefRO<LocalTransform> localTransform in SystemAPI.Query<RefRO<LocalTransform>>().WithAll<Boid>())
            {
                boidPositions.Add(localTransform.ValueRO.Position);
            }
            foreach (RefRO<Boid> boid in SystemAPI.Query<RefRO<Boid>>())
            {
                boidVelocities.Add(boid.ValueRO.Velocity);
            }
    ...
    public partial struct BoidJob : IJobEntity
    {
        ...
        List<float3> otherBoidPositions;
        List<float3> otherBoidVelocities;
